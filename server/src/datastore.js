...
module.exports = [
  ...
  get('/datastore/:id/:namespace/kinds/:kind/query', async ({ params: { id, namespace, kind }, query: { cursor } }) => {
    const datastore = getDatastore(id, namespace)
    const query = datastore.createQuery(kind).limit(100).start(cursor || null)
    const results = await datastore.runQuery(query)
    results[0].forEach(entity =>
      entity.__key__ = keyToKeyProto(entity[datastore.KEY]))
    return json(results)
  }),
  post('/datastore/:id/:namespace/delete', async ({ params: { id, namespace }, data }) => {
    const datastore = getDatastore(id, namespace)
    const keys = data.map(({path}) => datastore.key(path.map(({kind, id, name}) => ([
      kind,
